...
ConvWithSimValuesAndWeights = bool(int(sys.argv[2]))
K = int(sys.argv[3])
...
### 1 layer DCNN
dc_model = Sequential()
dc_model.add(DiffusionConv(filters=filters_1, prob_transition_mat = prob_transition_tensor,
...
dc_mfd_model.add(DiffusionConvModified(filters=filters_1, prob_transition_mat = prob_transition_tensor,
                        num_hops=K, activation='relu',
                        input_shape=(X_train.shape[1],1)))
