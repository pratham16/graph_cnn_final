...
class DiffusionConvModified(Layer):
    ...
    def __init__(self, 
        ...
        if K.backend() != 'tensorflow':
            raise Exception("DiffusionConvModified with Tensorflow Backend.")
            
        if 'input_shape' not in kwargs and 'input_dim' in kwargs:
           kwargs['input_shape'] = (kwargs.pop('input_dim'),)
           
        super(DiffusionConvModified, self).__init__(**kwargs)        
      
        self.filters = filters     
        self.num_hops = num_hops
        self.prob_transition_tensor = prob_transition_tensor
...
#        output = tf.tensordot(x_expanded, self.kernel, [[2,3],[0,1]])
        ...
        # Multiply prob_transition_tensor with similarity kernel
        self.prob_transition_tensor = tf.multiply(self.prob_transition_tensor, self.sim_kernel)
        soft_mask_mult = tf.tensordot(x, self.prob_transition_tensor, axes=[[1],[2]])
